...
namespace net{
    ...
    DataNode *MessageParser::createDataTreeFromLua(lua_State *lua,Node *node){
        ...
        //object类型以及普通数据节点。object类型的节点携带的数据为NULL，普通数据节点携带真正会被发送的数据。
        else{
            dataNode->data = val;
        }
    ...
    **/
    ...
    size_t MessageParser::serialDataNode(DataNode *node){
        size_t dataLen = 0;
        if ((node == NULL || node->data == NULL) && !node->isList && !(node->dataType == DataType::STRING)) {
            return 0;
        }
        switch (node->dataType) {
            case DataType::STRING:
            {
                ...
                int strLen = ret == NULL ? -1 : (int)strlen(ret);
    ...
    const void * MessageParser::getValueFromBinary(Node *node,int nodeTravelId,DataNode *parentNode){
        const void *ret = NULL;
        if (node->isList) {
            //如果当前正在解析的数据是一个list，则二进制信息中的前两个字节代表了List中所含元素的数量
            int16_t listLen = 0;
    ...
    void MessageParser::createLuaTable(DataNode *node){
        ...
        //如果遍历已经完毕，但lua栈上依然还有没有被嵌套设置的table则还要继续进行合并，直到合并成为一个根table为止
        if (node->isRoot) {
    ...
    /**
     *  将二进制数据转换为lua table，首先需要将二进制数据转换为data tree，然后再将data tree转换为lua table，也就是要将整个树上的信息按照其层次关系合并
